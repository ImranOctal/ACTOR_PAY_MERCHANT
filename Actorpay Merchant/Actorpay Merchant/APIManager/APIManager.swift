...
class APIManager {
    ...
    func uploadData (method : HTTPMethod,url:String, urlParameters:Parameters, bodyParameters: Parameters, imgData:Data?, imageKey:String, success:@escaping (APICompletionBlock)) {
        absoluteUrl = APIBaseUrlPoint.localHostBaseURL.rawValue + url
        let headers: HTTPHeaders = [.authorization(bearerToken: AppManager.shared.token)]
        print(headers)
        var param:Parameters? = urlParameters
        if let urlParameters = param {
            if !(urlParameters.isEmpty) {
                absoluteUrl.append("?")
                var array:[String] = []
